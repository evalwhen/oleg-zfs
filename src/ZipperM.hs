{-# LANGUAGE DeriveDataTypeable, ImpredicativeTypes, Haskell2010 #-}
...
traverse tf term = loop Nothing [] term
  where
  -- 每次调用 dzip'term 的 tf, 它都通过 shift 新建了一个新的 Dzipper 出来，这个 Dzipper
  -- 保存了遍历的（traverse) 的下一步动作（continuation), 被唤起的时候，它希望收到一个 NavigateDir
  -- 类型的数据，好判断下一步如何遍历（Update / Up / DownTo etc)
...
-- instance Typeable m => Typeable (DZipper m) where
...
pz :: Typeable m => Prompt PP m (DZipper m)
pz = pp

dzip'term :: (Typeable m, Monad m) => Term -> CC PP m (DZipper m)
dzip'term term = pushPrompt pz (traverse tf term >>= return . DZipDone . fst)
 ...
 dir_path = Prelude.foldr filterfn []
