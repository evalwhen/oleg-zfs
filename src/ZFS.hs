...
-- instance Typeable m => Typeable (OSReq m) where
...
pOS :: Typeable m => Prompt PP m (OSReq m)
pOS = pp
...
main' fs = bracket (serverSocket newClientPort) close $
  \s ->
    do
...
-- find'runnable 从 jobQueue 里面获取元素，可能的 job 元素类型见 JobQueueT
-- 包括 JQNewClient, JQBlockedOnRead, JQRunnable
-- 如果 find'runnable 找不到可以运行的 job, 那么整个 osloop 会阻塞在 wait'for'intr
-- 而 wait'for'intr 从 fds (listen socket or accepted socket) 里面 select ready
-- 的 fd，然后 try'to'run JobQueueT, 一共有三类 Job，客户端建连接，（shell)进程 block 在读取
-- 数据上, 最后是一个非阻塞的 job。
-- 可以看到，fs process 在想发起 IO 的时候，是向 os 发起 svc，此时 svc 利用 shift 等操作
-- 保存进程上下文，然后把它包装在 OSReq 类型里面，返回给 interpret'req 进行解释, 可以把它
-- 理解成 syscall handler。
-- interpret'req 会根据具体的请求类型来决定做什么，比如如果是 OSRRead (ReadK m)，它会 append
-- jobQuery，然后把新的 world 喂给 osloop 继续允许，自此 osloop 完成了一个循环。简单概述如下：

-- find'runnable -> try'to'run or wait'for'intr world.
-- 然后 注意 try'to'run 里面的 run'process 安装了一个 promp，以隔离进程和操作系统，它也是进程
-- 中断时的返回点，见以上 svc 如何包装一个 OSReq 

-- 具体文件系统的逻辑，见 fsProcess 的实现，它里面包含了对 DZipper 树的可暂停遍历，而且每个客户端
-- 看到的树可以不一样，但是这里如何落地为实际的文件系统呢，这里 DZipper 挂载的只是内存里面的树
...
-- Try to find the first runnable job
  ...
  wait'for'intr world@World{jobQueue=jq} =
    ...
    do fds  <- liftIO (sequence $ List.map toFD jq)
       let fds'n'jq = List.zip fds jq
       let mfd = catMaybes fds
       readyfd <- liftIO ( select'read'pending mfd >>=
                           either select_err return )
       case break (\(fd,_) -> maybe False (`elem` readyfd) fd) fds'n'jq of
          (jq,[]) -> return world -- nothing found
          (jq1,((_,now'runnable):jq2)) ->
             try'to'run now'runnable world{jobQueue=(List.map snd jq1)++(List.map snd jq2)}
   where
   toFD :: JobQueueT -> IO (Maybe Fd)
   toFD (JQNewClient s) = do
     fd <- unsafeFdSocket s
     return (Just (Fd fd))
   toFD (JQBlockedOnRead ProcessCTX{psocket=s} _) = do
     fd <- unsafeFdSocket s
     return (Just (Fd fd))
   toFD _ = return Nothing
  ...
  try'to'run qe@(JQNewClient s) world = do
      ...
      run'process (fsProcess (dzip'term (mountedFS world)))
	      >>= interpret'req (enqueue qe world) newCtx  -- 这里为什么要 enqueue
  ...
  try'to'run qe@(JQBlockedOnRead ctx@ProcessCTX{psocket=s} k) world = do
      ...
      dat <- liftIO $ (
	     do r <-  IO.tryIOError (recv s (1024 * 8))
		       ...
		       Right msg -> return (T.unpack (T.decodeUtf8 msg)))
...
interpret'req world ctx OSRDone = (liftIO . close $ psocket ctx)
				  >> return world
...
interpret'req world ctx (OSRWrite datum k) = do
 ...
 where
  send' _ ""  = return ()
  send' s msg = do  c <- liftIO $ send s (T.encodeUtf8 (T.pack msg))
                    send' s (List.drop c msg)
...
run'process :: (forall m. (Typeable m, Monad m) => CCM m (OSReq m))
	    -> CCM IO (OSReq IO)
...
-- TODO: 具体运行几个 cmd 来追踪运行，可以打点日志
fsloop :: (Typeable m, Monad m) => 
...
cmd'updnode :: (Typeable m, Monad m) =>
	      Term			-- new value
	      -> FSZipper m 
	      -> FileName
